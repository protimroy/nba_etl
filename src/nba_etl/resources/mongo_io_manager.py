...
class MongoPandasIOManager(IOManager):
    ...
    def handle_output(self, context, obj: pd.DataFrame):
        ...
        # Use asset key as collection name
        ...
        if docs:
            ...
            if key is None:
                ...
                for d in docs:
                    collection.update_one({key: d[key]}, {"$set": d}, upsert=True)
...
@io_manager(config_schema={"connection_str": str, "db_name": str})
def mongo_io_manager(init_context):
    return MongoPandasIOManager(
        init_context.resource_config["connection_str"],
        init_context.resource_config["db_name"],
    )
